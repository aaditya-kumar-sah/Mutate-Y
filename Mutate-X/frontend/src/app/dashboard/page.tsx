...
export default function HomePage() {
  ...
  // Load available genomes on mount
  useEffect(() => {
  ...
  // Load chromosomes when genome changes
  useEffect(() => {
    ...
    if (selectedGenome) fetchChromosomes();
  }, [selectedGenome]);
  ...
  // Auto-search when browsing chromosomes
  useEffect(() => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    performGeneSearch(searchQuery.trim(), selectedGenome);
  };
  ...
  const selectedGenomeSource = useMemo(
    () => genomes.find((g) => g.id === selectedGenome)?.sourceName,
    [genomes, selectedGenome]
  );

  return (
    <div className="min-h-screen bg-[#e9eeea] relative">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {chromosomes.map((chrom) => (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className={`h-8 cursor-pointer border-[#3c4f3d]/10 hover:bg-[#e9eeea] hover:text-[#3c4f3d] ${
                              selectedChromosome === chrom.name
                                ? "bg-[#e9eeea] text-[#3c4f3d]"
                                : ""
                            }`}
